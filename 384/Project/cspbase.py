...
class Variable: 
    ...
    def cur_domain_size(self):
        '''Return the size of the variable's domain (without constructing list)'''
        if self.is_assigned():
            return 1
        else:
    ...
    def assign(self, value):
        '''Used by bt_search. When we assign we remove all other
...
class Constraint: 
    ...
    def add_satisfying_tuples(self, tuples):
        ...
        for x in tuples:
            ...
            for i, val in enumerate(t):
                var = self.scope[i] # Variable object corresponding to i'th tuple entry
                if not (var,val) in self.sup_tuples:
                    self.sup_tuples[(var,val)] = [] # Initialize support
                self.sup_tuples[(var,val)].append(t) # Append new support for var,val pair
    ...
    def check(self, vals):
        '''Given list of values, one for each variable in the
           ...
           must be ordered in the same order as the list of
    ...
    def get_unasgn_vars(self): 
        '''return list of unassigned variables in constraint's scope. Note
           more expensive to get the list than to get the number'''
        vs = []
        for v in self.scope:
            if not v.is_assigned():
                vs.append(v)
    ...
    def has_support(self, var, val):
        '''Test if a variable value pair has a supporting tuple (a set
           ...
           still in the corresponding variable's current domain
        '''
...
class CSP:
    '''Class for packing up a set of variables into a CSP.
    ...
    def add_constraint(self,c):
        '''Add constraint to CSP. Note that all variables in the 
           constraint's scope must already have been added to the CSP'''
        if not type(c) is Constraint:
            print("Trying to add non constraint ", c, " to CSP object")
        else:
            for v in c.scope:
                if not v in self.vars_to_cons:
                    print("Trying to add constraint ", c, " with unknown variables to CSP object") # All variables in scope must already have been added
                    return
...
class BT:
    '''use a class to encapsulate things like statistics
       ...
       kind of propagator function to obtain plain backtracking,
    ...
    def restore_all_variable_domains(self):
        ...
        for var in self.csp.vars:
            if var.is_assigned(): # var's value is not None
                var.unassign() # Make var's value None
            var.restore_curdom() # Set var's curdom to all True
    ...
    def bt_search(self,propagator):
        '''Try to solve the CSP using specified propagator routine
           ...
           The list of variable value pairs are all of the values
    ...
    def bt_recurse(self, propagator, level):
        ...
        if not self.unasgn_vars:
            #all variables assigned (self.unasgn_vars is empty)
            return True
        else:
            var = self.extractMRVvar() # Extract unassigned variable with smallest current domain size
            if self.TRACE:
                print('  ' * level, "bt_recurse var = ", var)

            for val in var.cur_domain(): # In Slides 50-53, for each variable, iterate through each red value
                if self.TRACE:
                ...
                if self.TRACE: # Ran into dead end, so need to restore the current level's propagator's prunings
                    print('  ' * level, "bt_recurse restoring ", prunings)
                self.restoreValues(prunings)
                var.unassign() # Let var be assigned to the next value in its current domain

            self.restoreUnasgnVar(var) # Append var to unasgn_var for next extractMRVvar()
            return False
